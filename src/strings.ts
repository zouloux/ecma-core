import { ScalarRecord } from "./structs";
 ...
 */
export function indent ( total:number, content = '', tabSize = 0 ):string {
 ...
 */
export function parseQueryString (queryString:string):ScalarRecord {
	...
	// Convert string to number or boolean values
	function parseTypedValue ( value:any ) {
		// Check if a number first, so if '1' or '0', not converted to boolean
		if ( isNumber( value ) )
			return parseFloat( value )
		// Check if this can be a boolean
	...
	const output = {};
	queryString.split('&').map( tuples => {
		// Split on all =
		const split = tuples.split('=', 2);
		// If there is an =, this is a key/value
		output[ decodeURIComponent( split[0] ) ] = (
			( split.length === 2 )
			// Try to parse number from strings
			? parseTypedValue( decodeURIComponent( split[1] ) )
...
/**
 * Converts a string to an interpreted Boolean, for data-parsing usage.
 *
 * Return true
 * 		parseBoolean( 1 )
 * 		parseBoolean('1')
 ...
 */
export function parseBoolean ( booleanAsString:string|number, strict = true, areTrue = ['true'], areFalse = ['false'] ):boolean|null {
...
/**
 * Converts a string to an interpreted Boolean, for CLI usage.
 *
 * Return true
 * 		parseBoolean(1)
 * 		parseBoolean('1')
 ...
 * @param areFalse Lowercase values that are considered a false
 */
export function parseBooleanCLI ( booleanAsString:string|number, strict = true, areTrue = ['true', 'yes', 'y'], areFalse = ['false', 'no', 'n'] ):boolean|null {
	return parseBoolean( booleanAsString, strict, areTrue, areFalse )
}
